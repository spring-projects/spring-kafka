...
Messages can be received by configuring a `MessageListenerContainer` and providing a <<message-listener, Message Listener>>, or by
...
<1> Use this for processing individual `ConsumerRecord` s received from the kafka consumer `poll()` operations when
using auto-commit, or one of the container-managed <<committing-offsets, commit methods>>.

<2> Use this for processing individual `ConsumerRecord` s received from the kafka consumer `poll()` operations when
using one of the manual <<committing-offsets, commit methods>>.

<3> Use this for processing all `ConsumerRecord` s received from the kafka consumer `poll()` operations when
using auto-commit, or one of the container-managed <<committing-offsets, commit methods>>.
`AckMode.RECORD` is not supported when using this interface.
