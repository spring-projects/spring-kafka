...
Both asynchronous and synchronous mathods are provided, with the async methods returning a `Future`.
...
The third is similar to the second, but usese a regex `Pattern` to select the topics.
...
The consumer `poll()` method will return one or more `ConsumerRecords`; the `MessageListener` is calle for each record;
