...
The `ContainerStoppingErrorHandler` will stop the container if the listener throws an exception.
...
When the `AckMode` is `BATCH`, the entire batch will be replayed when the container is restarted.
