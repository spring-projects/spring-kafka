...
If your listener throws an exception, the default behavior is to invoke the `ErrorHandler`, if configured.
...
If a recovery callback is not provided, the exception is logged after retries are exhausted.
