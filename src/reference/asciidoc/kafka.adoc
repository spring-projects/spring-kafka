...
The sendDefault api's require that a default topic has been provided to the template.

[[record-read-timestamps]]
The API's which take in timestamp as a parameter will store this timestamp in the record. The behavior of the user provided timestamp is stored is dependent on the timestamp type configured on the Kafka topic.
