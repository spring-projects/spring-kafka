...
Starting with _versions 1.3.3_ the `org.springframework.core.convert.ConversionService` used by the default
...
of a listener method is provided all beans implementing any of the following interfaces:
...
----
@Bean
public class JAXBFooConverter implements Converter<String, Foo> {
    @Override
    public Foo convert(String source) {
        return JAXB.unmarshal(source, Foo.class);
    }
}
----
