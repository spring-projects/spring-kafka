...
abstraction with the `MessagingMessageConverter` implementation and its `JsonMessageConverter` (and subclasses) customization.
...
(`byte[]` and `Bytes` are more efficient because they avoid an unecessary `byte[]` to `String` conversion).
...
For convenience, the framework also provides a `StringOrBytesSerializer` which can serialize all three value types so it can be used with any of the message converters.
====
