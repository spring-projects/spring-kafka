...
In order to avoid a rebalance during an extended retry sequence, the error handler pauses, polls, and resumes the consumer before sleeping for the back off, for each retry, and calls the listener again.
If/when retries are exhausted, the `ConsumerRecordRecoverer` is called for each record in the batch.
If the recoverer throws an exception, a `SeekToCurrentErrorHandler` is invoked so that the batch of records will be redelivered on the next poll.
