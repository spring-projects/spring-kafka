...
Starting with version 2.6.4, a SpEL function `#parsePartitions` is registered to parse a list of ranges of partitions.
...
The function is only registered if bean factory is configured with the default `StandardBeanExpressionResolver`.
If you register your own bean expression resolver, and you wish to use this feature, you can register the function to reference the method `KafkaListenerAnnotationBeanPostProcessor.parsePartitions(String)` with the resolver's evaluation context.
