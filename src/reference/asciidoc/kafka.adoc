...
* if retries are exhausted and recovery fails, perform seeks as if retries are not exhausted.
...
public RecoveringBatchErrorHandler(KafkaTemplate<String, String> template) {
    ...
    RecoveringBatchErrorHandler errorHandler =
            new RecoveringBatchErrorHandler(recoverer, new FixedBackOff(3L, 5000));
}
----
====
