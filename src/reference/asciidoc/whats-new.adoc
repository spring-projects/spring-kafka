...
The `ContainerProperties` provides an `authorizationExceptionRetryInterval` option to let the listener container to retry after `AuthorizationException` is thrown by `KafkaConsumer`.
