...
The `brokerProperties` attribute of `@EmbeddedKafka` supports standard Spring property placeholder resolution from the `Environment`.  For example:
[source, java]
----
...
----
In this example, the property placeholders `${kafka.broker.logs-dir}` and `${kafka.broker.port}` are resolved from the Spring `Environment`.   The `Environment` contains system properties, the system environment, _and_ properties from the classpath resource `/kafka-broker.properties`.  All of these property sources are consulted when resolving property placeholders.  If the property placeholder is resolved by the `Environment`, the resolved value will be supplied to the broker, otherwise the original value will be supplied.

*_N.B._*: Property placeholders are _not_ supported for other attributes of `@EmbeddedKafka` at this time.
