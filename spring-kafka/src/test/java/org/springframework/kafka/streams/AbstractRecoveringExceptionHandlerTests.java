...
abstract class AbstractRecoveringExceptionHandlerTests<H, R> {
	...
	void withDestinationResolverAsStringProperty() {
		Map<String, Object> configs = new HashMap<>();
		configs.put(this.destinationResolverPropertyKey, NativeDlqDestinationResolver.class.getName());
		H handler = createHandler(configs);
		...
		assertThat(KafkaTestUtils.getPropertyValue(handler, "destinationResolver"))
				.isInstanceOf(NativeDlqDestinationResolver.class);
	...
	void withDestinationResolverAsClassProperty() {
		Map<String, Object> configs = new HashMap<>();
		configs.put(this.destinationResolverPropertyKey, NativeDlqDestinationResolver.class);
		H handler = createHandler(configs);
		ErrorHandlerContext context = createMockContext();
		R result = handleError(handler, context, new IllegalArgumentException());
		assertThat(KafkaTestUtils.getPropertyValue(handler, "destinationResolver"))
	...
	void withDestinationResolverAsObjectProperty() {
		Map<String, Object> configs = new HashMap<>();
		NativeDlqDestinationResolver resolver = new NativeDlqDestinationResolver();
	...
	public static class NativeDlqDestinationResolver implements NativeDeadLetterDestinationResolver {

		@Override
		public @NonNull TopicPartition apply(@NonNull ErrorHandlerContext errorHandlerContext,
				@NonNull ConsumerRecord<?, ?> record, @NonNull Exception exception) {
