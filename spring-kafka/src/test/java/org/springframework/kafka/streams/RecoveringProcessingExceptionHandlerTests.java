...
class RecoveringProcessingExceptionHandlerTests
	...
	protected ProcessingExceptionHandler.Response handleError(
		...
		ConsumerRecord<?, ?> consumerRecord = createExpectedConsumerRecord(context);
		return handler.handleError(context,
				new Record<>(consumerRecord.key(), consumerRecord.value(),
						consumerRecord.timestamp(), consumerRecord.headers()),
				exception);
	}
