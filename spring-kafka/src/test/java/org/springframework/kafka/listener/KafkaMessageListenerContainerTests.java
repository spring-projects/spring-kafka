...
//		props.put(ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG, 6); // 2 per poll
	...
	public void testSlowConsumerCommitsAreProcessed() throws Exception {
		...
		assertThat(latch.await(60, TimeUnit.SECONDS)).isTrue();
		InOrder inOrder = new InOrderImpl(Collections.singletonList(consumer));
		inOrder.verify(consumer, atLeastOnce()).pause(any(TopicPartition.class), any(TopicPartition.class));
		inOrder.verify(consumer, atLeastOnce()).commitSync(any());
		inOrder.verify(consumer, atLeastOnce()).resume(any(TopicPartition.class), any(TopicPartition.class));
	}
