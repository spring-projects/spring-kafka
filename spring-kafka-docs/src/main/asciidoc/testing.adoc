...
So, before running tests with an embedded Kafka on the random ports, we can set `spring.embedded.kafka.brokers.property=spring.kafka.bootstrap-servers` as system property - and `EmbeddedKafkaBroker`  will use it to expose its broker addresses.
