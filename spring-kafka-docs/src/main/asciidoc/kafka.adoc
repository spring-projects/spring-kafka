...
To do so, override the framework's `KafkaListenerAnnotationBeanPostProcessor` bean and add an `AnnotationEnhancer` to it.
`AnnotationEnhancer` is a `BiFunction<Annotation, AnnotatedElement, Annotation>` and must return an annotation.
The attributes can contain SpEL and/or property placeholders; the enhancer is called before any resolution is performed.
