...
Share consumers support four acknowledgment types:
 ...
 RENEW: Extend the acquisition lock (non-terminal; a terminal ack is still required when processing completes)
...
In explicit acknowledgment mode, the container enforces important constraints:
 ...
 You may call `renew()` zero or more times before that to extend the acquisition lock when processing exceeds the broker's lock duration.
...
In explicit acknowledgment mode, you must call ack.acknowledge(), ack.release(), or ack.reject() for every record.
Call ack.renew() to extend the lock; a terminal ack is still required.
----
