...
|[[awaitAsyncResultsOnStop]]<<awaitAsyncResultsOnStop,`awaitAsyncResultsOnStop`>>
|`false`
|When `true`, the container waits for in-flight asynchronous listener results (`CompletableFuture`, `Mono`, Kotlin `suspend`) to complete when it is stopped, bounded by the `shutdownTimeout`; results still outstanding when the timeout expires are cancelled and not acknowledged.
...
|[[consumerThreadExceptionHandler]]<<consumerThreadExceptionHandler,`consumerThreadExceptionHandler`>>
|`null`
|A handler for consumer exceptions thrown outside listener invocation.
When set, it is used instead of `CommonErrorHandler.handleOtherException()` for those exceptions; see xref:kafka/annotation-error-handling.adoc#consumer-thread-exception-handler[Consumer Thread Exception Handler].
...
Also bounds the wait for in-flight asynchronous listener results when `awaitAsyncResultsOnStop` is `true`.
