...
Starting with version 4.2.0, the listener container will automatically create and update Micrometer ``Counter``s for container start and stop events, if `Micrometer` is detected on the classpath, and a single `MeterRegistry` is present in the application context.
The counters can be disabled by setting the ``ContainerProperties``' `micrometerEnabled` to `false`.
...
The counters are maintained by the `KafkaMessageListenerContainer`.
Consequently, for a `ConcurrentMessageListenerContainer`, the counters are recorded per child container, each tagged with the child's bean name (`<name>-0`, `<name>-1`, and so on), rather than a single counter for the parent container.
