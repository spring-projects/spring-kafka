...
Also see xref:transaction-id-prefix[`transactionIdPrefix`].
...
This section refers to producer-only transactions (those not started by a listener container); see xref:container-transaction-manager[Using Consumer-Initiated Transactions] for information about chaining transactions when the container starts the transaction.
...
IMPORTANT: xref:retrytopic.adoc[Non-Blocking Retries] cannot combine with xref:container-transaction-manager[Container Transactions].
...
See the xref:kafka/annotation-error-handling.adoc#after-rollback[After-rollback Processor] for more information.
