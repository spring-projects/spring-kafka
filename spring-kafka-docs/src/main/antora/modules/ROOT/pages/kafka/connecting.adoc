...
* `ShareConsumerFactory` - see xref:kafka/kafka-queues.adoc[Kafka Queues (Share Consumer)]

Starting with version 2.5, each of these extends `KafkaResourceFactory`; the `ShareConsumerFactory` does so as of version 4.0.
...
Configure the `ABSwitchCluster` and add it to the producer, consumer and share consumer factories, and the `KafkaAdmin`, by calling `setBootstrapServersSupplier()`.
