...
In this mode, the most specific matching key in the class hierarchy is selected automatically, regardless of the map's iteration order.
Map order only matters when a value is assignable to two _unrelated_ keys (for example, two different interfaces); in that ambiguous case, provide an ordered `Map`, such as a `LinkedHashMap`, to make the tie-break deterministic.
