...
[[x41-share-consumer-lifecycle-events]]
...
See xref:kafka/kafka-queues.adoc#share-container-lifecycle-events[Lifecycle Events] for details.
...
This enables consistent exception classification across both single and batch processing when using xref:kafka/annotation-error-handling.adoc#default-eh[DefaultErrorHandler].
...
See xref:kafka/container-props.adoc#interceptBeforeTx[interceptBeforeTx].
...
See xref:kafka/receiving-messages/listener-annotation.adoc#batch-listeners[Batch Listeners] for more information.
...
See the note at the end of xref:kafka/receiving-messages/listener-annotation.adoc#batch-listeners[Batch Listeners] for more information.
...
See xref:kafka/configuring-topics.adoc[Configuring Topics] for more information.
...
See xref:kafka/seek.adoc[Seeking to a Specific Offset] for more information.
...
There is now an option to add a header which tracks delivery attempts when using certain error handlers and after rollback processors.
...
A new chapter xref:tips.adoc[Tips, Tricks and Examples] has been added.
...
See xref:kafka/receiving-messages/message-listeners.adoc[Message Listeners] for more information.
